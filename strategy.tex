...
We use VAE as the backbone for our EM-aware IR-drop prediction model, the structure is shown in Fig.\ref{fig:VAE_architecture}, we use a power grid with 64 rows and 64 columns as an example.
...
The input go through the VAE model from the encoder part. In this example, the encoder contains four convolutional layers and two fully-connected layers. 
The input is first padded to the standard size of 64 x 64 for the power grid smaller than this size. If the power grid exceeds the size of 64 x 64 but smaller than 128 x 128, then we add one more convolutional layer in both the generator and the discriminator network. 
