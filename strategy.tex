...
In the constraints, $s=[s_{1},s_{2},\ldots,s_{nt}]$  is the resizing factor for the power grid trees. $S$ is the feasible region defined in \eqref{eq:feasible_region}.
\begin{equation}
...
$ \dfrac{\partial v(t, s)}{\partial s}$ is the $n\times n_{t}$ \textit{Jacobian} matrix of $v(t,s)$ with respect to $s$, describes how the node voltage drops at target time $T=t$ respond to the resizing of power grid trees.
...
Each column in \eqref{eq:J_matrix} describes how will the nodes IR drop change due to the corresponding tree width rescaling. Every column has to be calculated through the entire process mentioned in \eqref{eq:dVs}. In comparison our VAE-based method constructs the \textit{Jacobian} matrix by conducting the inference procedure only once.
We utilize the PyTorch auto differentiation to acquire the gradient of output node voltage to the input wire segment conductance, which is $ \dfrac{\partial v(t, s)}{\partial g}$. Then we can easily get \eqref{eq:J_matrix} by chain rule, as each power grid tree $g_{i}$ has its own resizing factor $s_{i}$ and will not be infected by other resizing factors:
...
%After we use the above chain rule to fast compute the voltage drop $Jacobian$ matrix from the $GridNet$ sensitivity information,  we linearize the voltage drop in \eqref{eq:v_taylor_expand}, and formulate the \eqref{eq:prob_formulation} as a linear programming problem. Finally We find the solution of this linear programming problem by LP solver.
