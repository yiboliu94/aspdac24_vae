...
Recently Zhou {\it et al.} \cite{ZhouJin:ICCAD'20} considered more accurate physics-based EM effects into IR drop to build such sorrgate models. It adopted the GAN-based structure to model the resulting EM-aware IR drops.  This method regards the time, 2D power grid structure with input current and voltages as input images (series of images) and outputs the voltage map images. Such sorragate model can help to speedup wire sizing by fast EM-aware IR drop estimation and the sensitivity of objective functions with respect to the wire geometries. It was initially applied to localized PDN optimization then has been extended to full-chip optimization~\cite{HanLiu:TCAD'22-23}.
