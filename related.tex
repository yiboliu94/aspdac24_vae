...
% \subsection{Electromigration overview}
% \label{subsec:em}
% EM is a diffusion phenomenon of the migration of metal atoms due to several driving forces.  During migration, when the hydrostatic stress inside the embedded metal wire reaches the critical level, a void and a hillock will be formed by conducting electrons at the cathode end and the anode end.

% Some traditional EM models are developed based on empirical data. Black’s equation \cite{Black:1969fc} and Blech limit \cite{Blech:1976ko} are still employed in the industry. But there are growing criticism for the over-conservation of those models~\cite{}

% Recent studies show that large temperature gradient due to Joule heating may have huge impacts on the EM-induced failure effects, which can be the same magnitude as EM~\cite{Abbasinasab:DAC'2018,Guo:ICSSICTP'1998,Tu:JAP'19,KavousiTan:ICCAD'20}.  However, Joule heating and related TM effects were ignored in the existing EM-aware power grid fixing and optimization flow. 
